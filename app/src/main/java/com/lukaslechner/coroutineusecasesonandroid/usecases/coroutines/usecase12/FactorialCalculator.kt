...
class FactorialCalculator(
    ...
    suspend fun calculateFactorial(
        ...
        return withContext(Dispatchers.Default) {
            ...
            val calculatedSubRanges = subRanges.map { currentSubRange ->
                async { calculateFactorialOfSubRange(currentSubRange) }
            }.awaitAll()

            calculatedSubRanges.fold(BigInteger.ONE,  {acc, element -> acc.multiply(element) })
        }
    }
